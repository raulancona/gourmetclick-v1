...
function KanbanCard({ order, onClick, onAdvanceStatus }) {
    const items = Array.isArray(order.items) ? order.items : []

    // SLA coloring for the card border itself
    ...
    return (
        <div
            ...
            className={`bg-card rounded-xl p-3 mb-3 cursor-pointer transition-all hover:-translate-y-1 hover:shadow-lg shadow-sm border-l-4 ${isLate ? 'border-l-red-500 shadow-red-500/10' : 'border-l-primary/60'
        ...
        >
            ...
            <div className="flex items-center justify-between mt-auto pt-2 border-t border-border/50">
                ...
                <div className="flex gap-1.5">
                    {/* WhatsApp Action (if applicable for delivery or has phone) */}
...
function KanbanColumn({ title, emoji, color, orders, onClickOrder, onAdvanceStatus }) {
    return (
        <div className="flex flex-col flex-1 min-w-[280px] max-w-[320px] bg-muted/20 rounded-2xl p-2 border border-border/40 shrink-0">
...
export function KanbanBoard({ orders, onOrderClick, onAdvanceStatus }) {
    ...
    return (
        <div className="flex gap-4 overflow-x-auto custom-scrollbar pb-4 h-[calc(100vh-280px)] min-h-[500px]">
            {columns.map(col => {
